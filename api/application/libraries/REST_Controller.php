...
abstract class REST_Controller extends CI_Controller {
    ...
    //  echo 'ravvv'; die;
        ...
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name)))
        {
             ...
             */
            $userDataGlobal = $this->_get_userdata($this->_apiuser->Email, $this->_apiuser->Password,$this->_apiuser->UserType);
     ...
     */
    protected function createKey($userData, $password, $deviceId,$registerId)
    {
        // Build a new key
        $key = $this->_generate_key();

        // Insert the new key
        if ($this->_insert_key($key, $userData, $password, $deviceId,$registerId))
        {
            return $key;
        }
        else
     ...
     */
    protected function _get_userdata($username, $password,$userType)
    {
        $this->load->model('login');
        return $this->login->clientApiLogin($username,$password,$userType);
    }
	/**
    ...
    protected function _delete_key($id,$userId)
    {
      return $this->db
          ->where('id',$id)
          ->where('UserId', $userId)
     ...
     */
    protected function _insert_key($key, $userData, $password, $deviceId,$registerId)
    {
      //echo "<pre>"; print_r($userData); echo "</pre>"; die;
        ...
        $data['ip_addresses'] = $this->input->ip_address();
     ...
     */
    protected function _update_key($userId, $newKey=NULL, $deviceId,$registerId)
    {
        return $this->db
