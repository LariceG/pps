...
if (!function_exists('userDasboard')) {
    ...
    function searchStr($string,$find)
    {
      if(stripos($string,$find) || stripos($string,$find) == 0)
      return true;
    }
    ...
    function addScriptsFiles($page) {
        ...
        switch ($page) {
			...
			case 'selfassest':
                ...
                echo '<script type="text/javascript" src="' . site_url() . 'assets/js/jqueryAjax.js"></script>';
                echo '<script type="text/javascript" src="' . site_url() . 'assets/js/jquery-ui.min.js"></script>';
                $ci->load->view('client/selfassesments/selfassesment_js');
                break;
			case 'acc_selfassest':
                echo '<link rel="stylesheet" type="text/css" href="' . site_url() . 'assets/css/style_table.css"/>';
                echo '<link rel="stylesheet" type="text/css" href="' . site_url() . 'assets/css/cashman.css"/>';
                echo '<script type="text/javascript" src="' . site_url() . 'assets/js/jqueryAjax.js"></script>';
                echo '<script type="text/javascript" src="' . site_url() . 'assets/js/jquery-ui.min.js"></script>';
                $ci->load->view('accountant/selfassesments/aselfassesment_js');
                break;
			case 'viewacc_selfassest':
                echo '<link rel="stylesheet" type="text/css" href="' . site_url() . 'assets/css/style_table.css"/>';
                echo '<link rel="stylesheet" type="text/css" href="' . site_url() . 'assets/css/cashman.css"/>';
                echo '<script type="text/javascript" src="' . site_url() . 'assets/js/jqueryAjax.js"></script>';
                echo '<script type="text/javascript" src="' . site_url() . 'assets/js/jquery-ui.min.js"></script>';
...
// 		$query = $ci->db->get($prefix.'company');
    ...
    function sendEmail($email) {
        ...
        $ci->email->from(CASHMAN_FROM_EMAIL_ADDRESS, 'WIS Admin');
    ...
    function numberFormatExpenseExcel($number) {
        if (empty($number)) {
            return '';
        } else {
           // $number = str_replace('-', '', $number);
			$exact=floatval($number);
            //return '&pound;&nbsp;' . number_format($number, 2, '.', ',');
            return $exact;
        }
    }
    ...
    function update_trial_balance($type = "", $data = array(), $from = "", $tType = "", $action = "") {
        $ci = & get_instance();
        $user = $ci->session->userdata('user');
                    ...
                    if (!empty($invData)) {
                        ...
                        if ($from != "BANK" && $from != "BANK_DEL") {
                            ...
                            if ($user['VAT_TYPE'] == 'flat') {
                                ...
                                $govtVAT  =  $invData['InvoiceTax'];
                                $amount   =  $invData['InvoiceSub'];
                                //$amount   =  $invData["NetSales"];
                                //$invData["InvoiceTotal"] = $amount+$govtVAT;
                            }
                        ...
                        if (!empty($from)) {
                            if ($from == "BANK") {
                                if ($tType == "MONEY_IN") {
                                    ...
                                    if ((int) $TBentryId > 0) {
                                        add_trial_details($TBentryId, $tbSource, "CASH_AT_BANK", $aAccess, $invData["id"], $invData["InvoiceTotal"], $invData["PaidOn"]);
                                    }
                                    ...
                                    if ((int) $TBentryId > 0) {
                                        add_trial_details($TBentryId, $tbSource, "SALES_SUSPENSES", $aAccess, $invData["id"], $invData["InvoiceTotal"], $invData["PaidOn"], "SUBTRACT");
                                    }
                                    ...
                                    $TBentryId = store_trial_entry($TBCatId, $invData["PaidOn"], $invData["InvoiceTotal"], "SUBTRACT");
                                    if ((int) $TBentryId > 0) {
                                        add_trial_details($TBentryId, $tbSource, "CASH_AT_BANK", $aAccess, $invData["id"], $invData["InvoiceTotal"], $invData["PaidOn"], "SUBTRACT");
                                    }
                                    ...
                                    $TBentryId = store_trial_entry($TBCatId, $invData["PaidOn"], $invData["InvoiceTotal"]);
                                    if ((int) $TBentryId > 0) {
                                        add_trial_details($TBentryId, $tbSource, "SALES_SUSPENSES", $aAccess, $invData["id"], $invData["InvoiceTotal"], $invData["PaidOn"]);
                                    }
                                }
                                ...
                                if ($tType == "MONEY_IN") {
                                    ...
                                    $TBentryId = store_trial_entry($TBCatId, $invData["PaidOn"], $invData["InvoiceTotal"], "SUBTRACT");
                                    if ((int) $TBentryId > 0) {
                                        rm_trial_details($TBentryId, $tbSource, "CASH_AT_BANK", $invData["id"]);
                                    }
                                    ...
                                    $TBentryId = store_trial_entry($TBCatId, $invData["PaidOn"], $invData["InvoiceTotal"]);
                                    if ((int) $TBentryId > 0) {
                                        rm_trial_details($TBentryId, $tbSource, "SALES_SUSPENSES", $invData["id"]);
                                    }
                                } else if ($tType == "MONEY_OUT") {
                                    // "Cash at bank" goes down by "Invoice total amount"
                                    $TBCatId = get_trial_balance_category("CASH_AT_BANK"); //get category ID for given key
                            ...
                            if ($action == "DELETE") {
                                ...
                                if ((int) $TBentryId > 0) {
                                    rm_trial_details($TBentryId, $tbSource, "SALES", $invData["id"]);
                                }
                            } else {
                                $TBentryId = store_trial_entry($TBCatId, $invData["PaidOn"], $amount, "SUBTRACT");
                                if ((int) $TBentryId > 0) {
                                    add_trial_details($TBentryId, $tbSource, "SALES", $aAccess, $invData["id"], $amount, $invData["PaidOn"], "SUBTRACT");
                                }
                            }
                            ...
                            if ($action == "DELETE") {
                                ...
                                if ((int) $TBentryId > 0) {
                                    rm_trial_details($TBentryId, $tbSource, "VAT_CONTROL", $invData["id"]);
                                }
                            } else {
                                $TBentryId = store_trial_entry($TBCatId, $invData["PaidOn"], $govtVAT, "SUBTRACT");
                                if ((int) $TBentryId > 0) {
                                    add_trial_details($TBentryId, $tbSource, "SALES_SUSPENSES", $aAccess, $invData["id"], $govtVAT, $invData["PaidOn"], "SUBTRACT");
                                }
                            }
                        }
                        // die();
                        ...
                        if (!empty($from)) {
                            if ($from == "BANK") {
                                if ($tType == "MONEY_IN") {
                                    ...
                                    if ((int) $TBentryId > 0) {
                                        add_trial_details($TBentryId, $tbSource, "CASH_AT_BANK", $aAccess, $salData["id"], $salData["GrossSalary"]);
                                    }
                                    ...
                                    if ((int) $TBentryId > 0) {
                                        add_trial_details($TBentryId, $tbSource, "SALARY_SUSPENSE", $aAccess, $salData["id"], $salData["GrossSalary"], "SUBTRACT");
                                    }
                                } else if ($tType == "MONEY_OUT") {
                                    ...
                                    $TBentryId = store_trial_entry($TBCatId, $salData["PaidDate"], $salData["GrossSalary"], "SUBTRACT");
                                    if ((int) $TBentryId > 0) {
                                        add_trial_details($TBentryId, $tbSource, "CASH_AT_BANK", $aAccess, $salData["id"], $salData["GrossSalary"], "SUBTRACT");
                                    }
                                    ...
                                    $TBentryId = store_trial_entry($TBCatId, $salData["PaidDate"], $salData["GrossSalary"]);
                                    if ((int) $TBentryId > 0) {
                                        add_trial_details($TBentryId, $tbSource, "SALARY_SUSPENSE", $aAccess, $salData["id"], $salData["GrossSalary"]);
                                    }
                                }
                            } else if ($from == "BANK_DEL") {
                                ...
                                if ($tType == "MONEY_IN") {
                                    ...
                                    $TBentryId = store_trial_entry($TBCatId, $salData["PaidDate"], $salData["GrossSalary"], "SUBTRACT");
                                    if ((int) $TBentryId > 0) {
                                        rm_trial_details($TBentryId, $tbSource, "CASH_AT_BANK", $salData["id"]);
                                    }
                                    ...
                                    $TBentryId = store_trial_entry($TBCatId, $salData["PaidDate"], $salData["GrossSalary"]);
                                    if ((int) $TBentryId > 0) {
                                        rm_trial_details($TBentryId, $tbSource, "SALARY_SUSPENSE", $salData["id"]);
                                    }
                                } else if ($tType == "MONEY_OUT") {
                            ...
                            if ($action == "DELETE") {
                                $TBentryId = store_trial_entry($TBCatId, $salData["PaidDate"], $salData["GrossSalary"], "SUBTRACT");
                                if ((int) $TBentryId > 0) {
                                    rm_trial_details($TBentryId, $tbSource, $tbDetailType, $salData["id"]);
                                }
                            } else {
                                $TBentryId = store_trial_entry($TBCatId, $salData["PaidDate"], $salData["GrossSalary"]);
                                if ((int) $TBentryId > 0) {
                                    add_trial_details($TBentryId, $tbSource, $tbDetailType, $aAccess, $salData["id"], $salData["GrossSalary"]);
                                }
                            }
                            ...
                            if ($action == "DELETE") {
                                ...
                                if ((int) $TBentryId > 0) {
                                    add_trial_details($TBentryId, $tbSource, "SALARY_SUSPENSE", $aAccess, $salData["id"], $salData["NetPay"], "SUBTRACT");
                                }
                            }
                            ...
                            if ($action == "DELETE") {
                                ...
                                if ((int) $TBentryId > 0) {
                                    rm_trial_details($TBentryId, $tbSource, "PAYROLL_TAX_SUSPENSE", $salData["id"]);
                                }
                            } else {
                                $TBentryId = store_trial_entry($TBCatId, $salData["PaidDate"], $payRollSuspense, "SUBTRACT");
                                if ((int) $TBentryId > 0) {
                                    add_trial_details($TBentryId, $tbSource, "PAYROLL_TAX_SUSPENSE", $aAccess, $salData["id"], $payRollSuspense, "SUBTRACT");
                                }
                            }
                            ...
                            if ($action == "DELETE") {
                                ...
                                if ((int) $TBentryId > 0) {
                                    rm_trial_details($TBentryId, $tbSource, "EMPLOYERS_NI", $salData["id"]);
                                }
                            } else {
                                $TBentryId = store_trial_entry($TBCatId, $salData["PaidDate"], $salData["Employeer_NIC"]);
                                if ((int) $TBentryId > 0) {
                                    add_trial_details($TBentryId, $tbSource, "EMPLOYERS_NI", $aAccess, $salData["id"], $salData["Employeer_NIC"]);
                                }
                            }
                            ...
                            if ($action == "DELETE") {
                                ...
                                $TBentryId = store_trial_entry($TBCatId, $salData["PaidDate"], $salData["Employeer_NIC"], "SUBTRACT");
                                if ((int) $TBentryId > 0) {
                                    add_trial_details($TBentryId, $tbSource, "PAYROLL_TAX_SUSPENSE", $aAccess, $salData["id"], $salData["Employeer_NIC"], "SUBTRACT");
                                }
                            }
                        }
                    }
                        ...
                        if (!empty($from)) {
                            ...
                            if ($from == "BANK") {
                                if ($tType == "MONEY_OUT") {
                                    ...
                                    if ((int) $TBentryId > 0) {
                                        add_trial_details($TBentryId, $tbSource, "CASH_AT_BANK", $aAccess, $expData["id"], $expData["Amount"], $expData["PaidOn"], "SUBTRACT");
                                    }
                                    ...
                                    if ((int) $TBentryId > 0) {
                                        add_trial_details($TBentryId, $tbSource, $expData["ExpenseType"], $aAccess, $expData["id"], $expData["Amount"], $expData["PaidOn"]);
                                    }
                                    ...
                                    $TBentryId = store_trial_entry($TBCatId, $expData["PaidOn"], $expData["Amount"]);
                                    if ((int) $TBentryId > 0) {
                                        add_trial_details($TBentryId, $tbSource, "CASH_AT_BANK", $aAccess, $expData["id"], $expData["Amount"], $expData["PaidOn"]);
                                    }
                                    ...
                                    $TBentryId = store_trial_entry($TBCatId, $expData["PaidOn"], $expData["Amount"], "SUBTRACT");
                                    if ((int) $TBentryId > 0) {
                                        add_trial_details($TBentryId, $tbSource, $expData["ExpenseType"], $aAccess, $expData["id"], $expData["Amount"], $expData["PaidOn"], "SUBTRACT");
                                    }
                                }
                            } else if ($from == "BANK_DEL") {
                                if ($tType == "MONEY_OUT") {
                                    ...
                                    $TBentryId = store_trial_entry($TBCatId, $expData["PaidOn"], $expData["Amount"]);
                                    if ((int) $TBentryId > 0) {
                                        rm_trial_details($TBentryId, $tbSource, "CASH_AT_BANK", $expData["id"]);
                                    }
                                    ...
                                    $TBentryId = store_trial_entry($TBCatId, $expData["PaidOn"], $expData["Amount"], "SUBTRACT");
                                    if ((int) $TBentryId > 0) {
                                        rm_trial_details($TBentryId, $tbSource, $expData["ExpenseType"], $expData["id"]);
                                    }
                                } else if ($tType == "MONEY_IN") {
                                    // "Cash at bank" goes down by "Total amount"
                                    $TBCatId = get_trial_balance_category("CASH_AT_BANK"); //get category ID for given key
                                ...
                                foreach ($expData["items"] as $expItem) {
                                    ...
                                    if ($TBCatId) {
                                        ...
                                        if ($action == "DELETE") {
                                            ...
                                            if ((int) $TBentryId > 0) {
                                                rm_trial_details($TBentryId, $tbSource, $expItem["key"], $expItem["id"]);
                                            }
                                        } else {
                                            $TBentryId = store_trial_entry($TBCatId, $expData["PaidOn"], $expAmount);
                                            if ((int) $TBentryId > 0) {
                                                add_trial_details($TBentryId, $tbSource, $expItem["key"], $aAccess, $expItem["id"], $expAmount, $expData["PaidOn"]);
                                            }
                                        }
                                    }
                                    $expAmountTotal = $expAmount + $expItem["VATAmount"];
                                    /* Moved into loop */
                                    ...
                                    if ($action == "DELETE") {
                                        $TBentryId = store_trial_entry($TBCatId, $expData["PaidOn"], $expAmountTotal);
                                        if ((int) $TBentryId > 0) {
                                            rm_trial_details($TBentryId, $tbSource, $tbDetailType, $expItem["id"]);
                                        }
                                    } else {
                                        $TBentryId = store_trial_entry($TBCatId, $expData["PaidOn"], $expAmountTotal, "SUBTRACT");
                                        if ((int) $TBentryId > 0) {
                                            add_trial_details($TBentryId, $tbSource, $tbDetailType, $aAccess, $expItem["id"], $expAmountTotal, $expData["PaidOn"], "SUBTRACT");
                                        }
                                    }
                                    ...
                                    if ($standardClient) {
                                        $TBCatId = get_trial_balance_category("VAT_CONTROL");
                                        if ($action == "DELETE") {
                                            $TBentryId = store_trial_entry($TBCatId, $expData["PaidOn"], $expItem["VATAmount"], "SUBTRACT");
                                            if ((int) $TBentryId > 0) {
                                                //rm_trial_details($TBentryId, $tbSource, "VAT_CONTROL", $expData["id"]);
                                                rm_trial_details($TBentryId, $tbSource, "VAT_CONTROL", $expItem["id"]);
                                            }
                                        } else {
                                            $TBentryId = store_trial_entry($TBCatId, $expData["PaidOn"], $expItem["VATAmount"]);
                                            if ((int) $TBentryId > 0) {
                                                //add_trial_details($TBentryId, $tbSource, "VAT_CONTROL", $aAccess, $expData["id"], $expData["TotalVATAmount"]);
                                                add_trial_details($TBentryId, $tbSource, "VAT_CONTROL", $aAccess, $expItem["id"], $expItem["VATAmount"], $expData["PaidOn"]);
                                            }
                                        }
                                    }
                            ...
                            if ($standardClient) {
                                ...
                                if ($action == "DELETE") {
                                    ...
                                    if ((int) $TBentryId > 0) {
                                        //add_trial_details($TBentryId, $tbSource, "VAT_CONTROL", $aAccess, $expData["id"], $expData["TotalVATAmount"]);
                                        add_trial_details($TBentryId, $tbSource, "VAT_CONTROL", $aAccess, $expItem["id"], $expData["TotalVATAmount"]);
                                    }
                                }
                            }
                            */
                        ...
                        if (!empty($from)) {
                            ...
                            if ($from == "BANK") {
                                ...
                                if ($tType == "MONEY_IN") {
                                    ...
                                    if ((int) $TBentryId > 0) {
                                        add_trial_details($TBentryId, $tbSource, "CASH_AT_BANK", $aAccess, $payData["id"], $payData["Total"]);
                                    }
                                    ...
                                    if ((int) $TBentryId > 0) {
                                        add_trial_details($TBentryId, $tbSource, "PAYROLL_TAX_SUSPENSE", $aAccess, $payData["id"], $payData["Total"], "SUBTRACT");
                                    }
                                } else if ($tType == "MONEY_OUT") {
                                    ...
                                    $TBentryId = store_trial_entry($TBCatId, $payData["PaidDate"], $payData["Total"], "SUBTRACT");
                                    if ((int) $TBentryId > 0) {
                                        add_trial_details($TBentryId, $tbSource, "CASH_AT_BANK", $aAccess, $payData["id"], $payData["Total"], "SUBTRACT");
                                    }
                                    ...
                                    $TBentryId = store_trial_entry($TBCatId, $payData["PaidDate"], $payData["Total"]);
                                    if ((int) $TBentryId > 0) {
                                        add_trial_details($TBentryId, $tbSource, "PAYROLL_TAX_SUSPENSE", $aAccess, $payData["id"], $payData["Total"]);
                                    }
                                }
                            } else if ($from == "BANK_DEL") {
                                ...
                                if ($tType == "MONEY_IN") {
                                    ...
                                    $TBentryId = store_trial_entry($TBCatId, $payData["PaidDate"], $payData["Total"], "SUBTRACT");
                                    if ((int) $TBentryId > 0) {
                                        rm_trial_details($TBentryId, $tbSource, "CASH_AT_BANK", $payData["id"]);
                                    }
                                    ...
                                    $TBentryId = store_trial_entry($TBCatId, $payData["PaidDate"], $payData["Total"]);
                                    if ((int) $TBentryId > 0) {
                                        rm_trial_details($TBentryId, $tbSource, "PAYROLL_TAX_SUSPENSE", $payData["id"]);
                                    }
                                } else if ($tType == "MONEY_OUT") {
                        ...
                        if (!empty($from)) {
                            ...
                            if ($from == "BANK") {
                                ...
                                if ($tType == "MONEY_IN") {
                                    ...
                                    if ((int) $TBentryId > 0) {
                                        add_trial_details($TBentryId, $tbSource, "CASH_AT_BANK", $aAccess, $vatdata["id"], $vatdata["Total"]);
                                    }
                                    ...
                                    if ((int) $TBentryId > 0) {
                                        add_trial_details($TBentryId, $tbSource, "VAT_CONTROL", $aAccess, $vatdata["id"], $vatdata["Total"], "SUBTRACT");
                                    }
                                } else if ($tType == "MONEY_OUT") {
                                    ...
                                    $TBentryId = store_trial_entry($TBCatId, $vatdata["PaidDate"], $vatdata["Total"], "SUBTRACT");
                                    if ((int) $TBentryId > 0) {
                                        add_trial_details($TBentryId, $tbSource, "CASH_AT_BANK", $aAccess, $vatdata["id"], $vatdata["Total"], "SUBTRACT");
                                    }
                                    ...
                                    $TBentryId = store_trial_entry($TBCatId, $vatdata["PaidDate"], $vatdata["Total"]);
                                    if ((int) $TBentryId > 0) {
                                        add_trial_details($TBentryId, $tbSource, "VAT_CONTROL", $aAccess, $vatdata["id"], $vatdata["Total"]);
                                    }
                                }
                            } else if ($from == "BANK_DEL") {
                                ...
                                if ($tType == "MONEY_IN") {
                                    ...
                                    $TBentryId = store_trial_entry($TBCatId, $vatdata["PaidDate"], $vatdata["Total"], "SUBTRACT");
                                    if ((int) $TBentryId > 0) {
                                        rm_trial_details($TBentryId, $tbSource, "CASH_AT_BANK", $vatdata["id"]);
                                    }
                                    ...
                                    $TBentryId = store_trial_entry($TBCatId, $vatdata["PaidDate"], $vatdata["Total"]);
                                    if ((int) $TBentryId > 0) {
                                        rm_trial_details($TBentryId, $tbSource, "VAT_CONTROL", $vatdata["id"]);
                                    }
                                } else if ($tType == "MONEY_OUT") {
                        ...
                        if (!empty($from)) {
                            if ($from == "BANK") {
                                ...
                                if ($tType == "MONEY_IN") {
                                    ...
                                    if ((int) $TBentryId > 0) {
                                        add_trial_details($TBentryId, $tbSource, "CASH_AT_BANK", $aAccess, $divdata["id"], $divdata["Total"], $divdata["PaidOn"]);
                                    }
                                    ...
                                    if ((int) $TBentryId > 0) {
                                        add_trial_details($TBentryId, $tbSource, "DIVIDEND_SUSPENSE", $aAccess, $divdata["id"], $divdata["Total"], $divdata["PaidOn"], "SUBTRACT");
                                    }
                                } else if ($tType == "MONEY_OUT") {
                                    ...
                                    $TBentryId = store_trial_entry($TBCatId, $divdata["PaidDate"], $divdata["Total"], "SUBTRACT");
                                    if ((int) $TBentryId > 0) {
                                        add_trial_details($TBentryId, $tbSource, "CASH_AT_BANK", $aAccess, $divdata["id"], $divdata["Total"], $divdata["PaidOn"], "SUBTRACT");
                                    }
                                    ...
                                    $TBentryId = store_trial_entry($TBCatId, $divdata["PaidDate"], $divdata["Total"]);
                                    if ((int) $TBentryId > 0) {
                                        add_trial_details($TBentryId, $tbSource, "DIVIDEND_SUSPENSE", $aAccess, $divdata["id"], $divdata["Total"], $divdata["PaidOn"]);
                                    }
                                }
                            } else if ($from == "BANK_DEL") {
                                ...
                                if ($tType == "MONEY_IN") {
                                    ...
                                    $TBentryId = store_trial_entry($TBCatId, $divdata["PaidDate"], $divdata["Total"], "SUBTRACT");
                                    if ((int) $TBentryId > 0) {
                                        rm_trial_details($TBentryId, $tbSource, "CASH_AT_BANK", $divdata["id"]);
                                    }
                                    ...
                                    $TBentryId = store_trial_entry($TBCatId, $divdata["PaidDate"], $divdata["Total"]);
                                    if ((int) $TBentryId > 0) {
                                        rm_trial_details($TBentryId, $tbSource, "DIVIDEND_SUSPENSE", $divdata["id"]);
                                    }
                                } else if ($tType == "MONEY_OUT") {
                            ...
                            if ($action == "DELETE") {
                                ...
                                if ((int) $TBentryId > 0) {
                                    rm_trial_details($TBentryId, $tbSource, "PLA_EQUITY_DIVIDENDS", $divdata["id"]);
                                }
                            } else {
                                $TBentryId = store_trial_entry($TBCatId, $divdata["PaidDate"], $divdata["NetAmount"]);
                                if ((int) $TBentryId > 0) {
                                    add_trial_details($TBentryId, $tbSource, "PLA_EQUITY_DIVIDENDS", $aAccess, $divdata["id"], $divdata["NetAmount"], $divdata["PaidOn"]);
                                }
                            }
                            ...
                            if ($action == "DELETE") {
                                ...
                                $TBentryId = store_trial_entry($TBCatId, $divdata["PaidDate"], $divdata["NetAmount"], "SUBTRACT");
                                if ((int) $TBentryId > 0) {
                                    add_trial_details($TBentryId, $tbSource, "DIVIDEND_SUSPENSE", $aAccess, $divdata["id"], $divdata["NetAmount"], $divdata["PaidOn"], "SUBTRACT");
                                }
                            }
                            return;
                        }
    ...
    #Get Corporation_tax Function
    function get_corporation_tax_name(){
        $ci = & get_instance();
        $ci->load->database();
        $prefix = $ci->db->dbprefix;
        $query = $ci->db->query("SELECT SettingValue FROM " . $prefix . "configuration WHERE SettingName = 'Corporation_tax'");
        //echo $ci->db->last_query();
        if ($query->num_rows() > 0) {
            $r = $query->result();
            $settings = $r[0]->SettingValue;
            return $settings;
        }else{
            return $settings = '';
        }
    ...
    #Get Corporation_tax Function
    function get_corporation_tax_year(){
        $ci = & get_instance();
        $ci->load->database();
        $prefix = $ci->db->dbprefix;
        $query = $ci->db->query("SELECT SettingValue FROM " . $prefix . "configuration WHERE SettingName = 'Corporation_tax_year'");
        //echo $ci->db->last_query();
        if ($query->num_rows() > 0) {
            $r = $query->result();
            $settings = $r[0]->SettingValue;
    ...
    function getTBPaidStartEndDate($PaidDate, $year) {
        ...
        // Code working on current date
        $nxt["start"] = strtotime(($year) . "-" . $endDateMonth . "-" . $endDateDay) + (24 * 60 * 60);
        $nxt["end"] = strtotime(($year + 1) . "-" . $endDateMonth . "-" . $endDateDay);


        // Code working on previous
        /* $nxt["start"] = strtotime(($year - 1) . "-" . $endDateMonth . "-" . $endDateDay) + (24 * 60 * 60);
        $nxt["end"] = strtotime(($year) . "-" . $endDateMonth . "-" . $endDateDay); */
    ...
    function add_trial_details($TBentryId, $tbSource, $tbDetailType, $aAccess, $itemId, $amount,$itemDate = '', $action = "") {
    ...
    function rm_trial_details($TBentryId, $tbSource, $tbDetailType, $itemId) {
        $ci = & get_instance();
        $prefix = $ci->db->dbprefix;
        $where = array(
            "tbId" => $TBentryId,
	...
	function getparentQuestions($id){
		$ci = & get_instance();
        $prefix = $ci->db->dbprefix;
        $query = "SELECT * FROM " . $prefix . "selfassest_questions WHERE ID='" . $id . "'";
        $query = $ci->db->query($query);
        // echo $ci->db->last_query();//die;
        if ($query->num_rows() > 0) {
	...
	function getsubQuestionAnswer($id){
		$ci = & get_instance();
        $prefix = $ci->db->dbprefix;
        $query = "SELECT * FROM " . $prefix . "selfassest_answer WHERE ID='" . $id . "'";
        $query = $ci->db->query($query);
        // echo $ci->db->last_query();//die;
        if ($query->num_rows() > 0) {
            $data = $query->row_array();
	...
	function getComCompanyName(){
		$ci = & get_instance();
        $prefix = $ci->db->dbprefix;
        $query = "SELECT * FROM " . $prefix . "company";
        $query = $ci->db->query($query);
         //echo $ci->db->last_query();//die;
         if ($query->num_rows() > 0) {
            $data = array('0' => 'Select Company');
            foreach ($query->result() as $key => $val) {
                //$id = $this->encrypt->encode('ACTION_CHECK_USER/'.$val->ID);
                $data[$val->ClientID] = $val->Name;
            }
            return $data;
        } else {
            return array('0' => 'No Company');
        }
	}

/**************** Get all Adjustments made in CT  ****************/

	function get_adjustments($userid,$year){
		$ci = & get_instance();
		$ci->load->model('clients/ctadjustments');
		$result = $ci->ctadjustments->get_adjustment($userid,$year);
		$ci->session->set_userdata(array('adjustedct'=>$result));
		return $result;
	}
...
/////////////////////////////////////////////////////////////////////
	...
	function getReport_old(){
			...
			if ($query->num_rows() > 0) {
				foreach ($query->result() as $val) {
					$ci = & get_instance();
					$prefix = $ci->db->dbprefix;
					$query2 = "SELECT CID,Name FROM " . $prefix . "company WHERE ClientID ='" . $val->ID . "'";
					$query2 = $ci->db->query($query2);
					if ($query2->num_rows() > 0) {
						foreach ($query2->result() as $val) {
							$ci = & get_instance();
							$prefix = $ci->db->dbprefix;
							$query3 = "SELECT ID,DesignationType FROM  `cashman_company_customers`
							WHERE CompanyID ='" . $val->CID . "' AND DesignationType = 'E'";
							$query3 = $ci->db->query($query3);
							//echo $ci->db->last_query();die;
							if ($query3->num_rows() > 0) {
								foreach ($query3->result() as $val) {
									$ci = & get_instance();
									$prefix = $ci->db->dbprefix;
									$query5 = "SELECT ID, SUM( Amount ) FROM  `cashman_expense_items`
									WHERE ExpenseID ='" . $val->ID . "' AND ItemType = 'MILEAGE' GROUP BY ExpenseID";
									$query5 = $ci->db->query($query5);
									//echo $ci->db->last_query();die;
    ...
    function getReport(){
        ...
        if ($query->num_rows() > 0) {
                foreach ($query->result() as $val) {
                        $ci = & get_instance();
                        $prefix = $ci->db->dbprefix;
                        $query2 = "SELECT CID,Name FROM cashman_company WHERE ClientID ='" . $val->ID . "'";
                        $query2 = $ci->db->query($query2);
                        if ($query2->num_rows() > 0) {
                                foreach ($query2->result() as $val) {
                                        $ci = & get_instance();
                                        ...
                                        if ($query3->num_rows() > 0) {
                                                foreach ($query3->result() as $val) {
                                                        ...
                                                        $ci = & get_instance();
                                                        $prefix = $ci->db->dbprefix;
                                                        $query4 = "SELECT ID FROM `cashman_expenses` WHERE `EmployeeID` = ".$val->ID;
                                                        $query4 = $ci->db->query($query4);
                                                        //echo $ci->db->last_query();//die;
                                                        if ($query4->num_rows() > 0) {
                                                           $expenseIDS = "";
                                                            foreach ($query4->result() as $val) {
                                                                 if( $expenseIDS == '')
                                                                      $expenseIDS =  $val->ID;
                                                                 else
                                                                     $expenseIDS .=  ",".$val->ID;
                                                       ...
                                                       //echo "<pre>"; print_r($expenseIDS); die;

                                                        $query5 = "SELECT e.EmployeeID, SUM( i.Amount ) as total FROM  cashman_expense_items as i
                                                        INNER JOIN cashman_expenses as e
                                                        ON e.ID = i.ExpenseID
                                                        WHERE i.ExpenseID IN (".$expenseIDS . ") AND i.ItemType = 'MILEAGE' GROUP BY i.ExpenseID";
                                                        $query5 = $ci->db->query($query5);
                                                        //echo $ci->db->last_query();die;
                                                        if ($query5->num_rows() > 0) {
                                                                $totalAmount = 0;
                                                                ...
                                                                if($totalAmount > 10000 ) {
                                                                   ...
                                                                   $rrr = array();
                                                                   $rrr['Emp'] =  $val->EmployeeID;
                                                                   $rrr['total'] =   $totalAmount;
                                                                   $report[] = $rrr;
                                                                   if(!in_array( $val->EmployeeID,$reportEmp)){
                                                                          $reportEmp[] =  $val->EmployeeID;
                                                                          if($reportEmpStr == "")
                                                                              $reportEmpStr = $val->EmployeeID;
                                                                          else
                                                                               $reportEmpStr .= ",".$val->EmployeeID;
                                                                   }
                                                                }
     ...
     */
    function get_car_previous_miles($id, $date) {
        $prefix = $ci->db->dbprefix;
        $ci = & get_instance();
        ...
        */
        if (strtotime($date) < strtotime($date_check)) {
            $f_start_date = (date('Y', strtotime($date)) - 1) . '-04-05';
            $f_end_date = $date;
        ...
        $query = "SELECT e.EmployeeID FROM cashman_expense_items as i INNER JOIN cashman_expenses as e ON e.ID = i.ExpenseID WHERE i.ID=".$id;
        $query = $ci->db->query($query);
        $r = $query->result();
        $empID = $r[0]->EmployeeID;

        $query = "SELECT i.Miles FROM cashman_expense_items AS i";
        $query .= " LEFT JOIN cashman_expenses AS e ON e.ID = i.ExpenseID ";
        $query .= " WHERE i.ItemType='MILEAGE' AND i.Category=32 AND e.EmployeeID=" . $empID;
    ...
    function getBankVatableCat() {
        $ci = & get_instance();
        $prefix = $ci->db->dbprefix;
        $ci->db->select('ID');
        $query = $ci->db->get_where('expenses_category', array('CategoryType' => 'BANK', 'vatable' => 1));
        if ($query->num_rows() > 0) {
            $result = $query->result();
            $catList = array();
    ...
    // Get Quarter NAme By Item Date
    function getVatQuaterByDate($d, $quaters){
            //echo "**********<br>";pr($quaters);echo "</br>";
            $vatQuarterInfo = array();
            if(strtotime($d) >= strtotime($quaters[1]['FIRST']) && strtotime($d)  <= strtotime($quaters[1]['SECOND'])){
                $vatQuarterInfo['q'] = 1;
                $vatQuarterInfo['start'] = $quaters[1]['FIRST'];
                $vatQuarterInfo['end'] = $quaters[1]['SECOND'];

            }else if(strtotime($d) >= strtotime($quaters[2]['FIRST']) && strtotime($d)  <= strtotime($quaters[2]['SECOND'])){
                $vatQuarterInfo['q'] = 2;
                $vatQuarterInfo['start'] = $quaters[2]['FIRST'];
                $vatQuarterInfo['end'] = $quaters[2]['SECOND'];

            }else if( strtotime($d) >= strtotime($quaters[3]['FIRST']) && strtotime($d) <= strtotime($quaters[3]['SECOND'])){
                $vatQuarterInfo['q'] = 3;
                $vatQuarterInfo['start'] = $quaters[3]['FIRST'];
                $vatQuarterInfo['end'] = $quaters[3]['SECOND'];

            }else if( strtotime($d) >= strtotime($quaters[4]['FIRST'])  && strtotime($d) <= strtotime($quaters[4]['SECOND']) ){
                $vatQuarterInfo['q'] = 4;
                $vatQuarterInfo['start'] = $quaters[4]['FIRST'];
                $vatQuarterInfo['end'] = $quaters[4]['SECOND'];

            }

        return $vatQuarterInfo;
    }
    ...
    function getBankStatmentLastUploadDate($clientID = null, $cID = null) {
        $ci         = & get_instance();
        $prefix     = $ci->db->dbprefix;
        //create by bluk email but stop in wisonline
        //$query      = $ci->db->query('SELECT TransactionDate FROM `'.$prefix.'bank_statements` where AddedBy ='.$clientID.' AND compnayID='.$cID.' ORDER BY `TransactionDate` DESC');
        $query      = $ci->db->query('SELECT TransactionDate FROM `'.$prefix.'bank_statements` where AddedBy ='.$clientID.' AND compnayID='.$cID);
        if ($query->num_rows() > 0) {
    ...
    /* getVatMarkPaidQuarters function use for when invoice and Expenses paid. It's returs VAT Quters and Financial Year create by sb */
    function getVatMarkPaidQuarters($vDate = NULL)
    {
        $ci = & get_instance();
        $prefix = $ci->db->dbprefix;
        ...
        if ($row = $result->row())
        {
            ...
            $startYear  = ($year-1);
            $endYear    = ($year+1);
            for ($runYear=$startYear; $runYear<=$endYear; $runYear++)
            {
                $year = $runYear;
                for ($i = 1; $i <= 4; $i++)
                {
                    // can be MAX 4 Quarters only
                    $sQuater = "q" . $i . "_start";
                    $eQuater = "q" . $i . "_end";
                    ...
                    if (strtotime($vatQuaters[$i]["FIRST"])<=strtotime($vDate) && strtotime($vatQuaters[$i]["SECOND"])>=strtotime($vDate))
                    {
                        $vatQuaterAndYear["year"] = $runYear;
                        $vatQuaterAndYear["quter"] = $i;
                        break;
                    }
                }
            }
        }
        //echo "<pre>"; print_r( $vatQuaterAndYear ); die();
        return $vatQuaterAndYear;
    }

    /*
    * getClientVatQuarters function use for get Quarter and year by client and Company id. It's returs VAT Quters and Financial Year create by sb
    * $clientId   UserId
    * $companyId  CompanyId
    * $firstDate  Paid Date (27-08-2017)
    */
    function getClientVatQuarters($clientId,$companyId,$firstDate)
    {
        $ci = & get_instance();
        $prefix = $ci->db->dbprefix;
        $vatQuaters         = false;
        ...
        $ci->db->select('*');
        $result = $ci->db->get_where('vat_quarters', array('ClientID' => $clientId,'companyID' => $companyId));
        if ($row = $result->row())
        {
            ...
            for ($runYear=$startYear; $runYear<=$endYear; $runYear++)
            {
                ...
                for ($i = 1; $i <= 4; $i++)
                {
                    ...
                    if (strtotime($vatQuaters[$i]["FIRST"])<=strtotime($firstDate) && strtotime($vatQuaters[$i]["SECOND"])>=strtotime($firstDate))
                    {
                        $vatQuaterAndYear["year"] = $runYear;
                        $vatQuaterAndYear["quarter"] = $i;
                        break;
                    }
                }
            }
    ...
    /*
    * getClientVatMonthBulkEmail function use for get select month to get data create by Sb
    * $clientId   UserId
    * $companyId  CompanyId
    * $firstDate  Select Date Format (27-08-2017)
    * $status     Select Vat Status (0 = Waiting(Default), 1 = Approved, 2 = Rejected, 3 = Awaiting Approval, 4 = Submitted)
    * $userData   Return Match user data
    */
    function getClientVatMonthBulkEmail($clientId,$companyId,$firstDate, $status, $userData)
    {
        //echo "<pre>getClientVatMonthBulkEmail";
        //var_dump($userData);
        //die();
        $ci = & get_instance();
        $prefix = $ci->db->dbprefix;
        $vatQuaters         = false;
        $getMatchVatMonthUsers   = false;
        ...
        if ($row = $result->row())
        {
            ...
            for ($runYear=$startYear; $runYear<=$endYear; $runYear++)
            {
                ...
                for ($i = 1; $i <= 4; $i++)
                {
                    ...
                    {
                        // echo "<br><br>";
                        // echo $getMatchVatMonthUsers[$i]["FIRST"];
                        // echo "<br>";
                        // echo $getMatchVatMonthUsers[$i]["SECOND"];
                        // echo "<br>";
                        // echo "<br>";
                        if ($runYear===$year)
                        {
                            $getMatchVatMonthUsers["matchUser"] = $userData;
                            $getMatchVatMonthUsers["year"] = $runYear;
                            $getMatchVatMonthUsers["quarter"] = $i;
                            break;
                        }
                    }
                }
        ...
        //echo "<pre>"; print_r( $getMatchVatMonthUsers ); //die();
        return $getMatchVatMonthUsers;
    }


	#Get All Parent	From Trial Balance Category Table
	function getCategoryParent($d){
		$ci = &get_instance();
		$prefix = $ci->db->dbprefix;
		$ci->db->select('id,title');
		$query = $ci->db->get('cashman_trial_balance_categories');
		$ci->db->order_by("listingOrder", "asc");
		$query = $ci->db->get_where('cashman_trial_balance_categories', array('parentType =' => 2));
		//echo $ci->db->last_query(); die;
		$result = $query->result();
		...
		#Get All Child Using Parent ID
	function getCategoryParentChild($parentId){
		$ci = &get_instance();
		$prefix = $ci->db->dbprefix;
		$user = $ci->session->userdata('user');
		if($parentId == 235){ #customer child categry
			$ci->db->select('id,catKey');
			$ci->db->order_by("catKey", "asc");
			$query = $ci->db->get_where('cashman_trial_balance_categories', array('AnalysisLedgerParent =' => $parentId));
			$result = $query->result();
			//echo $ci->db->last_query(); die;
			$customerList = array();
			if ($query->num_rows() > 0) {
				$i=0;
				foreach($result as $key => $val) {
					//$ci->db->select('id,CONCAT(first_name, '.', last_name) AS title');
					$ci->db->select('id,companyname AS title');
					//$ci->db->order_by('id', 'asc');
					$query2 = $ci->db->get_where('cashman_customers', array('TB_Category =' => $val->catKey,'ClientID'=>$user['UserID'],'ClientCID'=>$user['CompanyID'],'status'=>1));
					$result2 = $query2->result();
					//echo $ci->db->last_query(); //die;
					if ($query2->num_rows() > 0) {
						$result2[0]->id = $val->id;
						$customerList[$i] = $result2[0];
						$i++;
					}
		...
		}elseif($parentId == 242){ #supplier child categry
			...
			if ($query->num_rows() > 0) {
				...
				foreach($result as $key => $val) {
					...
					//$ci->db->order_by('id', 'asc');
					$query2 = $ci->db->get_where('cashman_suppliers', array('TB_Category =' => $val->catKey,'ClientID'=>$user['UserID']));
					$result2 = $query2->result();
					//echo $ci->db->last_query(); //die;
					if ($query2->num_rows() > 0) {
						$result2[0]->id = $val->id;
						$result2[0]->title = $result2[0]->first_name." ".$result2[0]->last_name;
						$supplierList[$i] = $result2[0];
						$i++;
					}
		...
		}elseif($parentId == 279){ #Shareholder child categry
			$ci->db->select('id,catKey');
			$ci->db->order_by("catKey", "asc");
			$query = $ci->db->get_where('cashman_trial_balance_categories', array('AnalysisLedgerParent =' => $parentId));
			$result = $query->result();
			...
			if ($query->num_rows() > 0) {
				...
				foreach($result as $key => $val) {
					...
					//$ci->db->order_by('id', 'asc');
					$query2 = $ci->db->get_where('cashman_company_customers', array('TbCategoryshareholder =' => $val->catKey,'CompanyID'=>$user['CompanyID'],'IS_ShareHolder'=>1));
					$result2 = $query2->result();
					//echo "<br><br><br>".$ci->db->last_query();
					if ($query2->num_rows() > 0) {
						$result2[0]->id = $val->id;
						$result2[0]->title = $result2[0]->FirstName." ".$result2[0]->LastName;
						$shareholdersList[$i] = $result2[0];
						$i++;
					}
				}
			} else {
		...
		}elseif($parentId == 286){ #Employee child categry
			$ci->db->select('id,catKey');
			$ci->db->order_by("catKey", "asc");
			$query = $ci->db->get_where('cashman_trial_balance_categories', array('AnalysisLedgerParent =' => $parentId));
			$result = $query->result();
			...
			if ($query->num_rows() > 0) {
				...
				foreach($result as $key => $val) {
					$ci->db->select('id,FirstName,LastName');
					$query2 = $ci->db->get_where('cashman_company_customers', array('TbCategoryEmployee =' => $val->catKey,'CompanyID'=>$user['CompanyID'],'IS_Employee'=>1));
					$result2 = $query2->result();
					if($query2->num_rows() > 0) {
						$result2[0]->id = $val->id;
						$result2[0]->title = $result2[0]->FirstName." ".$result2[0]->LastName;
						$employeeList[$i] = $result2[0];
						$i++;
					}
				}
			} else {
  ...
  function sendNotification($data) {
      ...
      $fields = array(
          ...
          'data' => $data['message'],
          'priority' => 'high',
          'notification' => array('sound' => 'default', 'title' => 'Data Frog', 'body' => $message['title'])
      );
      $headers = array(
