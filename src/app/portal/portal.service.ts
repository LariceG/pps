...
export class PortalService {
  ...
  getAdpmStores(apdm,type)
  {
    let headers = new Headers();
    headers.append('ApiKey',this.token['apiKey']);
    return this.http.get(myGlobals.baseUrl+'api/getAdpmStores/'+apdm+'/'+type, { headers }).map(
          (res: Response) => res.json() || {});
  }

  apdmUserListing(page,type,perpage)
  {
    let headers = new Headers();
    headers.append('ApiKey',this.token['apiKey']);
    return this.http.get(myGlobals.baseUrl+'api/apdmUserListing/'+page+'/'+perpage+'/'+type,{headers}).map(
          (res: Response) => res.json() || {});
  }
